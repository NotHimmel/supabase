...
const getRecentItemHref = (ref: string, item: RecentItem) => {
  switch (item.type) {
    case 'sql':
      return `/project/${ref}/sql/${item.metadata?.sqlId}`
    case 'notebook':
      return `/project/${ref}/explorer/notebook/${item.metadata?.notebookId}`
    case 'query':
      return `/project/${ref}/explorer/query/${item.metadata?.queryId}`
    case 'r':
    case 'v':
    case 'm':
    case 'f':
    ...
    default:
      return `/project/${ref}/explorer/${item.type}/${item.metadata?.schema}/${item.metadata?.name}`
  }
}
          ...
          <AnimatePresence>
            <div className="grid grid-cols-1 gap-12 gap-y-0">
              {sortedItems.map((item, index: number) => (
                ...
                >
                  <Link
                    href={getRecentItemHref(ref, item)}
